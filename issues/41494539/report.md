...
### he...@gmail.com (2024-01-25)

This is triggered during loading webpage of my fuzzer. However, I fail to reproduce it again after 2 hours running, seems that the reproduction is pretty flaky.

However, after the investigation, the root cause is clear, i..e, this is related to the lacking race protection of `reject_writes_` in `ChannelPosix::ShutDownOnIOThread`. This leading to the same handle accessed between the different thread, causing UAF as the stack trace shows.

There lacks a `write_lock_` protection to `reject_writes_` and the resetting handles by `write_watcher_.reset()`. Meanwhile, this is manifested in [0]

[0] https://chromium-review.googlesource.com/c/chromium/src/+/5226127

After looking other code base among the ipcz, I find that the `ShutDownOnIOThread` in channel_fuchsia still face this UAF issue, since it doesn't set `reject_writes_` to true during the shutdown on IO thread in [1].

[1] https://source.chromium.org/chromium/chromium/src/+/main:mojo/core/channel_fuchsia.cc;l=267-277;drc=70a6711e08e9f9e0d8e4c48e9ba5cab62eb010c2

Hence we still need a fix on the [1], adding lock protection and set `reject_writes_` to true as [0] does.

### he...@gmail.com (2024-01-25)

Moreover, I find there exists a residual UAF in the libchrome ipcz, which lacks the `write_lock_` protection in [2]. Hence we need the similar patch towards [2], adding the missing lock here.
...
void ChannelPosix::ShutDownOnIOThread() {
  ...
  read_watcher_.reset();
  write_watcher_.reset(); // [2] resetting write_watcher_ handles used by WriteNoLock which is called in another thread, leading to the racing access
  if (leak_handle_) {
...
[2] https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/libchrome/mojo/core/channel_posix.cc;l=249-258;drc=eb8baeaf6154dc50b95139fd165b922b179d4742



### li...@chromium.org (2024-01-25)

pbos@ could you take a look at this? It looks like the data race failure from crbug.com/1519980 might cause a UAF.

This does look fixed on channel_posix but this race might still happen on other platforms such as fuchsia.
Also I think the fix should be merged into the extended stable release since this is now a security bug.
...
### ro...@google.com (2024-01-25)

The Channel implementations are completely independent, so unless there is some evidence of UAFs on other platforms, I think we should assume this is only a Linux+ChromeOS issue.

### pb...@chromium.org (2024-01-25)

Hey, looks like tsan on the main waterfall would've saved us here. Neat.

The fix in #4, crrev.com/c/5226127 hasn't made it into src/platform/libchrome/mojo. +cc jorgelo@ can we trigger a mojo roll in libchrome and uh how do we merge stuff like this to stable in libchrome? We should probably merge it to (extended) stable for both chromium/src and libchrome.
...
After another 10 hours fuzzing on the ToT chromium, I could still trigger this UAF memory corruption issue, which means the https://crrev.com/c/5226127 doesn't fix this actually.

I attached the PoC and the latest asan.txt on 123.0.6265.0 Linux chromium. In addition, due to the sanitizer marking limitation, that accessed memory is partly exceed the freed memory boundary, leading to this ASan stack says this is a heap-buffer-overflow issue. That's said, not only can it access freed memory, but access the exceeded boundary memory as well, which actually a UAF issue. Meanwhile, it also demonstrates that this UAF could be trigger by pure web page loading, i.e., sandboxed renderer, should be a ** critical severity ** issue.

### he...@gmail.com (2024-01-26)

deleted

### ro...@google.com (2024-01-26)

Sorry, I hadn't actually looked at the ASAN reports until now, but can we keep this bug focused on those (both from https://crbug.com/chromium/1521571#c1 and https://crbug.com/chromium/1521571#c10)? These look like a UAF on a shared memory wrapper object.

Unless I'm misunderstanding, they seem completely unrelated to the ChannelPosix UAF mentioned in https://crbug.com/chromium/1521571#c3 and elsewhere, which is https://crbug.com/chromium/1519980 and which *should* in fact be fixed by https://crrev.com/c/5226127.

### ro...@google.com (2024-01-26)

This is caused by a weak debug assertion in DriverMemory::Clone(), which can fail e.g. if the driver bumps into FD limits[1]. We have:
...
In production if DuplicateSharedMemory (effectively just dup() on POSIX) fails, `handle` remains uninitialized. We go on to wrap it with a new DriverMemory which will then attempt to dereference its garbage value as a pointer.

[1] https://source.chromium.org/chromium/chromium/src/+/main:third_party/ipcz/src/ipcz/driver_memory.cc;drc=a9f577a03395a05b1d9f443d3dcdf1885e733567;l=51

### ro...@google.com (2024-01-26)

Fix is up at https://crrev.com/c/5240312 in case you want to give this another go with the fuzzer while that's still in flight.

### ro...@google.com (2024-01-26)

The UAF here is incidental to the more general problem of UB in dereferencing an uninitialized pointer.

### he...@gmail.com (2024-01-26)

Thanks for the quick fix. I'll run it again on the ToT chromium.

Moreover, for the residual fix mentioned in the https://crbug.com/chromium/1521571#c3# and https://crbug.com/chromium/1521571#c8, i.e., the patch on libchrome, should I file another new issue?

### ha...@google.com (2024-01-26)

[Empty comment from Monorail migration]

### pb...@chromium.org (2024-01-26)

Re #16, I am guessing that libchrome is not actually directly involved here and I got confused. Hopefully no action required.

### ro...@google.com (2024-01-26)

For this bug, no merge to libchrome would be useful because ipcz is still not enabled on Chrome OS.

For the ChannelPosix bug, a merge could be useful. IIUC libchrome rolls multiple times per week. Not sure about release branches though.

### aj...@chromium.org (2024-01-26)

This smells a bit like https://crbug.com/chromium/1508753 - or are there platform specifics?

### ro...@google.com (2024-01-26)

I would kind of expect a UAF caused by this bug to have either a use-stack in ipcz DriverMemory (or adjacent code), or a free-stack in ~DriverMemory(). But maybe not. I still haven't made sense of that other bug.

### gi...@appspot.gserviceaccount.com (2024-01-26)

The following revision refers to this bug:
  https://chromium.googlesource.com/chromium/src/+/4bd18c5a3a7a935716bbed197fba6d45a1122894

commit 4bd18c5a3a7a935716bbed197fba6d45a1122894
Author: Ken Rockot <rockot@google.com>
Date: Fri Jan 26 21:53:06 2024
...
[modify] https://crrev.com/4bd18c5a3a7a935716bbed197fba6d45a1122894/third_party/ipcz/src/ipcz/driver_memory.cc
[modify] https://crrev.com/4bd18c5a3a7a935716bbed197fba6d45a1122894/third_party/ipcz/src/ipcz/driver_transport.cc
...
Requesting merge to extended stable M120 because latest trunk commit (1252882) appears to be after extended stable branch point (1217362).

Requesting merge to stable M121 because latest trunk commit (1252882) appears to be after stable branch point (1233107).

Requesting merge to beta M122 because latest trunk commit (1252882) appears to be after beta branch point (1250580).
...
### he...@gmail.com (2024-01-28)

After running the PoC and the fuzzer with another multiple hours (~20h), I could verify that this is fixed.

### ro...@google.com (2024-01-29)

1. CL to merge: https://chromium.googlesource.com/chromium/src/+/4bd18c5a3a7a935716bbed197fba6d45a1122894
2. Yes verified and no regressions
3. No real potential for stability risks, strictly a memory bug fix
4. No compatibility risks for any milestone
5. No manual verification or testing required

### am...@chromium.org (2024-01-30)

merges approved forhttps://crrev.com/c/5240312
please merge to M122 branch 6261 as soon as possible so this fix can be included in next M122 beta
please merge M121 Stable  branch 6167 and M120 Extended Stable branch 6099 by EOD Thursday so this fix can be included in the next M121 Stable and M120 Extended security updates 


### gi...@appspot.gserviceaccount.com (2024-01-30)

The following revision refers to this bug:
  https://chromium.googlesource.com/chromium/src/+/c6d3fdc608e8684c1aa769d24865dfdb5b1d3ee6

commit c6d3fdc608e8684c1aa769d24865dfdb5b1d3ee6
Author: Ken Rockot <rockot@google.com>
Date: Tue Jan 30 20:39:34 2024

[M122] ipcz: Fix a few weak asserts
...
Reviewed-on: https://chromium-review.googlesource.com/c/chromium/src/+/5249574
Bot-Commit: Rubber Stamper <rubber-stamper@appspot.gserviceaccount.com>
Cr-Commit-Position: refs/branch-heads/6261@{#306}
Cr-Branched-From: 9755d9d81e4a8cb5b4f76b23b761457479dbb06b-refs/heads/main@{#1250580}

[modify] https://crrev.com/c6d3fdc608e8684c1aa769d24865dfdb5b1d3ee6/third_party/ipcz/src/ipcz/driver_memory.cc
[modify] https://crrev.com/c6d3fdc608e8684c1aa769d24865dfdb5b1d3ee6/third_party/ipcz/src/ipcz/driver_transport.cc
...
The following revision refers to this bug:
  https://chromium.googlesource.com/chromium/src/+/7f600356eb5659d0e35b7773d43ec2bdc33e443e

commit 7f600356eb5659d0e35b7773d43ec2bdc33e443e
Author: Ken Rockot <rockot@google.com>
Date: Tue Jan 30 21:04:49 2024

[M121] ipcz: Fix a few weak asserts
...
[modify] https://crrev.com/7f600356eb5659d0e35b7773d43ec2bdc33e443e/third_party/ipcz/src/ipcz/driver_memory.cc
[modify] https://crrev.com/7f600356eb5659d0e35b7773d43ec2bdc33e443e/third_party/ipcz/src/ipcz/driver_transport.cc


### gi...@appspot.gserviceaccount.com (2024-01-30)

The following revision refers to this bug:
  https://chromium.googlesource.com/chromium/src/+/a60fca19f6e90e27b5d41bb184b76940ad09ee85

commit a60fca19f6e90e27b5d41bb184b76940ad09ee85
Author: Ken Rockot <rockot@google.com>
Date: Tue Jan 30 21:36:01 2024
...
[modify] https://crrev.com/a60fca19f6e90e27b5d41bb184b76940ad09ee85/third_party/ipcz/src/ipcz/driver_memory.cc
[modify] https://crrev.com/a60fca19f6e90e27b5d41bb184b76940ad09ee85/third_party/ipcz/src/ipcz/driver_transport.cc
...
Congratulations! The Chrome VRP Panel has decided to award you $5,000 for this mildly mitigated security bug in a non-sandboxed process-- mitigated by race condition and unreliability to trigger. Thank you for your efforts and reporting this issue to us! 

### he...@gmail.com (2024-02-02)
...
This issue was migrated from crbug.com/chromium/1521571?no_tracker_redirect=1
...
### vo...@google.com (2024-02-08)

1. <https://crrev.com/c/5273991>
2. Low - no conflicts
3. M120
4. Yes

### na...@google.com (2024-02-08)

Approving merge for LTS-114

### ap...@google.com (2024-03-06)

Project: chromium/src
Branch: refs/branch-heads/5735

commit 3d516fd71511ef7d14f81f3f2cb71ed43f350b24
Author: Ken Rockot <rockot@google.com>
Date:   Wed Mar 06 11:45:55 2024

    [M114-LTS] ipcz: Fix a few weak asserts
    ...
    (cherry picked from commit a60fca19f6e90e27b5d41bb184b76940ad09ee85)
    
    Fixed: 1521571
    Change-Id: Id764b33ead8bbba58e61b3270920c839479eaa4a
    Reviewed-on: https://chromium-review.googlesource.com/c/chromium/src/+/5240312
    Commit-Queue: Ken Rockot <rockot@google.com>
    Cr-Original-Original-Commit-Position: refs/heads/main@{#1252882}
    Reviewed-on: https://chromium-review.googlesource.com/c/chromium/src/+/5250958
    Bot-Commit: Rubber Stamper <rubber-stamper@appspot.gserviceaccount.com>
    Auto-Submit: Ken Rockot <rockot@google.com>
    Cr-Original-Commit-Position: refs/branch-heads/6099@{#1905}
    Cr-Original-Branched-From: e6ee4500f7d6549a9ac1354f8d056da49ef406be-refs/heads/main@{#1217362}
    Reviewed-on: https://chromium-review.googlesource.com/c/chromium/src/+/5273991
    Commit-Queue: Victor Gabriel Savu <vsavu@google.com>
    Reviewed-by: Victor Gabriel Savu <vsavu@google.com>
    Owners-Override: Victor Gabriel Savu <vsavu@google.com>
    Cr-Commit-Position: refs/branch-heads/5735@{#1710}
    Cr-Branched-From: 2f562e4ddbaf79a3f3cb338b4d1bd4398d49eb67-refs/heads/main@{#1135570}

M       third_party/ipcz/src/ipcz/driver_memory.cc
M       third_party/ipcz/src/ipcz/driver_transport.cc

https://chromium-review.googlesource.com/5273991
