...
{  
   ...
   vrend_resource_reference(&target->buffer, res);           <----- Increase reference count res.base.reference.count  
  
   ret_handle = vrend_renderer_object_insert(ctx, target, handle,  
                                             VIRGL_OBJECT_STREAMOUT_TARGET);  
   if (ret_handle == 0) {  
      FREE(target);                <----- Should decrement the reference count res.base.reference.count here, but it doesn't  
      return ENOMEM;  
   }  
   return 0;  
}  
...
index 72dab234..2e36c663 100644  

--- a/src/vrend\_renderer.c  

+++ b/src/vrend\_renderer.c  

@@ -11544,6 +11544,7 @@ int vrend\_create\_so\_target(struct vrend\_context \*ctx,  

ret\_handle = vrend\_renderer\_object\_insert(ctx, target, handle,  

VIRGL\_OBJECT\_STREAMOUT\_TARGET);  
...
Privileges and Capabilities: Potential for privilege escalation. Need IOCTL privileges.
...
Severity assessment: Medium. There's no immediate demonstration in the PoC that you could use as a root chain or remotely or from a surface.
...
This issue was migrated from crbug.com/chromium/1478444?no_tracker_redirect=1
