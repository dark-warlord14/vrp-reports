...
| Field | Value |
|-------|-------|
| **Issue ID** | [40086457](https://issues.chromium.org/issues/40086457) |
| **Status** | Assigned |
| **Severity** | S3-Low |
| **Priority** | P1 |
...
When parsing 'SBGP' box of MP4 in file box\_definitions.cc, integer overflow happens.
...
entries is defined as std::vector<SampleToGroupEntry> entries;, entries.resize(count); will malloc count \* sizeof(SampleToGroupEntry) bytes, this will also overflow, leading to out-of-bounds write.

Because SBGP is in encrypted video, playing encrypted video requires deploying a whole system using Encrypted Media Extensions(EME). So I will not provide a PoC file, just patch file.

PATCH:  

--- box\_definitions\_a.cc 2017-01-10 14:53:55.000000000 +0800  

+++ box\_definitions\_b.cc 2017-01-10 15:18:42.000000000 +0800  
...
```
   uint32_t count;  
   RCHECK(reader->Read4(&count));  
+  RCHECK(((count \* sizeof(SampleToGroupEntry)) / sizeof(SampleToGroupEntry)) == count);  
   entries.resize(count);  
   for (uint32_t i = 0; i < count; ++i) {  
     RCHECK(reader->Read4(&entries[i].sample_count) &&  
...
- [patch.diff](attachments/patch.diff) (application/octet-stream, 411 B)
...
This issue was migrated from crbug.com/chromium/679646?no_tracker_redirect=1

[Monorail components added to Component Tags custom field.]

---
*Data from [Chromium Issue Tracker](https://issues.chromium.org/issues/40086457)*
