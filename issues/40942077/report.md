...
| Field | Value |
|-------|-------|
| **Issue ID** | [40942077](https://issues.chromium.org/issues/40942077) |
| **Status** | Assigned |
| **Severity** | S3-Low |
| **Priority** | P1 |
...
heap-buffer-overflow in libavif when decode the crafted avif file. cause out-of-bounds writing bug.
...
#0 0x7ff41ca5b943 in avifMetaFindAlphaItem libavif/src/read.c:4284  

#1 0x7ff41ca5e3e0 in avifDecoderReset libavif/src/read.c:4687  

#2 0x7ff41ca5a42c in avifDecoderParse libavif/src/read.c:4107  

#3 0x562930df8b09 in main libavif/apps/avifdec.c:335  

#4 0x7ff41c642082 in \_\_libc\_start\_main (/lib/x86\_64-linux-gnu/libc.so.6+0x24082)  

#5 0x562930df6acd in \_start (libavif/build/avifdec+0xaacd)

SUMMARY: AddressSanitizer: heap-buffer-overflow libavif/src/read.c:4284 in avifMetaFindAlphaItem
...
- [poc](attachments/poc) (text/plain, 2.3 KB)
- [bug-1501766-tentative.txt](attachments/bug-1501766-tentative.txt) (text/plain, 878 B)
        ...
        if (item->dimgForID == colorItem->id) {
            for (uint32_t j = 0; j < meta->items.count; ++j) {
                ...
                if (avifDecoderItemIsAlphaAux(auxlItem, item->id)) {
                    alphaItemIndices[alphaItemCount++] = j; // may be need some check.
                }
            }
        }
    }
...
This also reproduces in M119. Triaging to libavif owners.
...
Vignesh: This bug was introduced in "Allow alpha to be specified as an auxl of color tiles":
...
I attached a tentative patch from my initial investigation, mainly to pinpoint the location of the bug (see also https://crbug.com/chromium/1501766#c2).
...
Upstream fix in flight: https://github.com/AOMediaCodec/libavif/pull/1756

I will pull it into chromium once the fix is merged upstream.
...
Given that there are no issues with the roll of libavif on canary so far with this fix and having reviewed the upstream commit (https://github.com/AOMediaCodec/libavif/pull/1756/files) approving the CP of this fix for backmerge to 120 Beta / branch 6099, 119 Stable/ branch 6045, and 118 Extended Stable / branch 5993 at your earliest convenience. 
We are in release freeze this week, so please just merge the CPs for to the appropriate branch at your earliest convenience (by EOD Friday) so this fix can be included in the next security updates for each. Thanks!  
...
Congratulations on another one! The Chrome VRP Panel has decided to award you $7,000 for this report of a bug that allows for memory corruption in a sandboxed process. A member of our finance team will be in touch with you soon to arrange payment. Thank you for your efforts and reporting this issue to us -- nice work!
...
This issue was migrated from crbug.com/chromium/1501766?no_tracker_redirect=1
...
---
*Data from [Chromium Issue Tracker](https://issues.chromium.org/issues/40942077)*
